...
AudioGraphManager::AudioGraphManager()
{
    ...
    audioInputNode  = graph.addNode (std::make_unique<AudioProcessor>(new AudioGraphIOProcessor(AudioGraphIOProcessor::audioInputNode)));
    audioOutputNode  = graph.addNode (std::make_unique<AudioProcessor>(new AudioGraphIOProcessor(AudioGraphIOProcessor::audioOutputNode)));
...
{
    return (int)graph.addNode(std::make_unique<AudioProcessor>(instance))->nodeID.uid;
}
